...
class CycleGAN(nn.Module):
    def __init__(self, opt=None, mode='train'):
        ...
        self.lambda_B = opt.labmda_B
        self.netG_A = Generator(opt.input_nc, opt.output_nc)
        self.netG_B = Generator(opt.input_nc, opt.output_nc)
        if mode == 'train':
            self.netD_A = Discriminator(opt.output_nc)
            ...
            self.optimizer_G = optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D = optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self, real_A, real_B):
        ...
        loss_D_A = self.backward_D_A(fake_B)
        loss_D_B = self.backward_D_B(fake_A)
    ...
    def backward_D_A(self, fake_B):
        fake_B = self.fake_B_pool.query(self.fake_B)
        loss_D_A = self.backward_D_basic(self.netD_A, self.real_B, fake_B)
        return loss_D_A

    def backward_D_B(self, fake_A):
    ...
    def backward_G(self, real_A, real_B, fake_A, fake_B, rec_A, rec_B):
        ...
        lambda_B = self.labmda_B

        # Identity loss
        idt_A = self.netG_A(realB)
        loss_idt_A = self.criterionIdt(idt_A, real_B) * lambda_B * lambda_idt
        idt_B = self.netG_B(realA)
